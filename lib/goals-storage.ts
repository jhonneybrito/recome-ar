...
const EVENT = "recomecar:goals-updated";
...
export const saveGoals=(items:GoalRecord[])=>{if(typeof window==="undefined"||isSupabaseConfigured)return;window.localStorage.setItem(KEY,JSON.stringify(items));window.dispatchEvent(new Event(EVENT))};
...
export function useGoals(){
  ...
  useEffect(()=>{
    if(isSupabaseConfigured){setGoals([]);setInitialized(false);const syncRemote=()=>{getGoalsDb().then((remote)=>{setGoals(remote||[]);setInitialized(true);setReady(true)}).catch((error)=>{console.error(error);setReady(true)})};syncRemote();window.addEventListener(EVENT,syncRemote);return()=>window.removeEventListener(EVENT,syncRemote);}
  ...
  const upsertGoal=useCallback(async(goal:Omit<GoalRecord,"id">,id?:string)=>{if(isSupabaseConfigured){await saveGoalDb(goal,id);await refreshRemote();window.dispatchEvent(new Event(EVENT));return}const current=loadGoals();const next=id?current.map((item)=>item.id===id?{...goal,id}:item):[{...goal,id:crypto.randomUUID()},...current];setGoals(next);saveGoals(next);syncPatrimony(next)},[refreshRemote]);
  const removeGoal=useCallback(async(id:string)=>{if(isSupabaseConfigured){await deleteGoalDb(id);await refreshRemote();window.dispatchEvent(new Event(EVENT));return}const next=loadGoals().filter((item)=>item.id!==id);setGoals(next);saveGoals(next)},[refreshRemote]);
