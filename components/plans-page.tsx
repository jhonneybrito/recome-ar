...
export default function PlansPage() {
  ...
  const checkout = async (plan: string) => {
    const priceId = plan === "premium_monthly" ? process.env.NEXT_PUBLIC_STRIPE_MONTHLY_PRICE_ID : process.env.NEXT_PUBLIC_STRIPE_ANNUAL_PRICE_ID;
    if (!priceId) { setError("Configure o Price ID deste plano na Vercel para iniciar a assinatura."); return; }
    setLoading(plan); setError(""); trackEvent("InitiateCheckout", { content_name: plan });
    const response = await fetch("/api/stripe/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ priceId, plan }) });
    const data = await response.json();
    if (data.url) window.location.href = data.url;
    else { setError(data.message || "Não foi possível iniciar o checkout."); setLoading(""); }
  };
